...
export default function VideoPage() {
  ...
  if (error) {
    return <ErrorState message={error} onRetry={fetchVideoData} />;
  }
  ...
  const { subject, video, prev, next } = context || { 
    subject: { title: '...', slug: '' }, 
    video: { title: '...', duration: '...', youtubeUrl: '', quick_summary: '', roadmap: [], topicsCovered: [], notes: [] },
    prev: null, 
    next: null 
  };
    ...
    <div className="min-h-screen pb-32 relative overflow-hidden">
      ...
      <section className="pt-32 px-6 max-w-7xl mx-auto relative z-10">
        <Link to={loading || !subject?.slug ? "#" : `/subject/${subject.slug}`} className="inline-flex items-center gap-2 text-gray-500 hover:text-accent-blue mb-8 transition-colors group px-4 py-2 glass-card border-white/5 text-[10px] font-black uppercase tracking-widest">
          <ArrowLeft size={14} className="group-hover:-translate-x-1 transition-transform" /> Back to {loading ? "Subject" : subject.title}
        </Link>
        ...
        <div className="grid lg:grid-cols-12 gap-12 mb-20">
          ...
          >
            {loading ? (
              <div className="animate-pulse">
                <div className="aspect-video bg-gray-800 rounded-[2.5rem] mb-4"></div>
          ...
          >
            ...
            <h1 className="text-3xl md:text-4xl font-black mb-6 italic tracking-tighter leading-tight uppercase">
              {loading ? <div className="h-10 bg-white/10 rounded w-full animate-pulse"></div> : video.title}
            </h1>
            ...
            <div className="grid grid-cols-2 gap-4 mb-10">
              <div className="glass-card p-4 border-white/5 bg-white/[0.02]">
                 ...
                 <div className="text-xl font-black text-white flex items-center gap-2">
                    <Clock size={16} className="text-accent-blue" /> {loading ? <div className="h-6 w-12 bg-white/5 rounded animate-pulse"></div> : video.duration}
                 </div>
              </div>
              <div className="glass-card p-4 border-white/5 bg-white/[0.02]">
                 <div className="text-[9px] font-black text-gray-500 uppercase tracking-widest mb-1">Subject</div>
                 <div className="text-xl font-black text-white flex items-center gap-2">
                     <Target size={16} className="text-accent-purple" /> {loading ? <div className="h-6 w-20 bg-white/5 rounded animate-pulse"></div> : subject.title}
                 </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-2 space-y-12">
             <motion.section {...fadeInUp}>
                ...
                <GlassCard className="bg-white/[0.01] border-white/5 font-light text-xl leading-relaxed text-gray-300 italic">
                  "{video?.quick_summary || (video?.notes?.[0]?.content?.split('.')[0]) || "No summary available."}"
                </GlassCard>
             </motion.section>
                  ...
                  <div className="prose prose-invert max-w-none">
                     <div className="text-gray-400 text-lg leading-relaxed font-light space-y-6">
                        {loading ? (
                          ...
                          (video.notes[0].content || "").split('. ').map((para, i) => (
                            <p key={i}>{para}.</p>
                          ))
                        ) : (
                          <EmptyState 
                            title="Notes Encrypted" 
                            message="No mission notes detected for this module yet. Detailed documentation pending."
                            type="neutral"
                            action={<div/>}
                          />
                        )}
          ...
          <aside className="space-y-12">
               ...
               <div className="glass-card p-10 border-accent-purple/30 bg-accent-purple/[0.02] relative overflow-hidden">
                  ...
                  <h3 className="text-2xl font-black italic mb-8 glow-text tracking-tight uppercase flex items-center gap-3">
                   ...
                   <ul className="space-y-6">
                     {(video?.notes?.[0]?.tags ? video.notes[0].tags.split(',') : []).map((point, i) => (
            ...
            <motion.section {...fadeInUp}>
                <h3 className="text-xl font-black italic mb-6 opacity-60 uppercase tracking-widest">Subject Roadmap</h3>
                 <div className="space-y-4">
                   {loading ? (
                     [...Array(5)].map((_, i) => <SkeletonSubject key={i} />)
                   ) : (
                     (context?.subjectVideos || []).map((step, i) => (
                       <div 
                         ...
                         onClick={() => step.id !== parseInt(id) && navigate(`/video/${step.id}`)}
                       >
                         <div className="flex items-center gap-4">
                           <div className={cn(
                             "w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold border",
                             step.id === parseInt(id) ? "bg-accent-blue border-accent-blue text-white" : "border-gray-700 text-gray-500"
                           )}>
                             {i + 1}
                           </div>
                           <span className={cn(
                             "text-sm font-bold uppercase tracking-tight",
                             step.id === parseInt(id) ? "text-accent-blue" : "text-gray-500 group-hover:text-gray-300"
                           )}>
                             {step.title}
                           </span>
                         </div>
                         {step.id === parseInt(id) && <Play size={14} className="text-accent-blue animate-pulse" />}
                       </div>
                     ))
                   )}
                 </div>
        ...
        <section className="mt-32 pt-20 border-t border-white/5">
           ...
           <div className="grid md:grid-cols-3 gap-8">
              {loading ? (
                ...
                (context?.subjectVideos || []).filter(v => v.id !== parseInt(id)).slice(0, 3).map((v) => (
